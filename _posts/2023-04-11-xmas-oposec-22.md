...
So we got ourselves a little APL dark magic. I smelled this from afar due to its usage in (_CodeGolf_)[https://en.wikipedia.org/wiki/Code_golf]. The code was missing a newline, but apart from that it was just copy and paste in an online interpreter: 
...
And we have our first PIN, 0000 _puff_... After guessing the PIN, the script enters in interactive mode, so we can do the rest manually:
